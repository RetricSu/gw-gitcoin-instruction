...
In this step you must make a deposit of CKBytes from Layer 1 to the Layer 2 which is provided by Godwoken. This step is necessary in order for Godwoken to allocate resources to an account before it can be interacted with.

This can be accomplished using the example script code provided in the tutorial below. Make sure you have your private key from the previous step available since it will be needed by the example script.
