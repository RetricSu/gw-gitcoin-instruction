...
```js
cd ~projects/blockchain-workshop-ethereum-simple
yarn add @polyjuice-provider/web3@0.0.1-rc5 nervos-godwoken-integration@0.0.2
```
...
Godwoken requires the the price of gas be set to zero for all transactions. This may not always be the case in the future, but it is a requirement for the current version on the Testnet.

To accomodate for this, we can make a simple change to default the gas price to zero for the user when they make transactions. In our project, this is all handled in the file `SimpleStorageWrapper.ts`. Open the file `~projects/blockchain-workshop-ethereum-simple/src/lib/SimpleStorageWrapper.ts` in your editor.

First, we define a simple object that contains the `gas` and `gasPrice` keys used by MetaMask.
...
When deploying a contract, one additional change needs to be made. The `to` key must be set to a zero-filled address. For example, this code:
...
Just below the above code, we will find this line:

```js
this.useDeployed(contract._address);
```

We need to use the `contractAddress` property of the transaction, so the line is changed to:
...
### View the Completed Godwoken Demo Application
