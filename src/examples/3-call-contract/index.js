...
// const CONTRACT_ABI = '[     {       "inputs": [],       "stateMutability": "payable",       "type": "constructor"     },     {       "inputs": [         {           "internalType": "uint256",           "name": "x",           "type": "uint256"         }       ],       "name": "set",       "outputs": [],       "stateMutability": "payable",       "type": "function"     },     {       "inputs": [],       "name": "get",       "outputs": [         {           "internalType": "uint256",           "name": "",           "type": "uint256"         }       ],       "stateMutability": "view",       "type": "function"     }   ]';
// const CONTRACT_ADDRESS = '<YOUR_CONTRACT_ADDRESS';
...
(async () => {
    ...
    if (balance === 0n) {
        console.log(`Insufficient balance. Can't issue a smart-contract call. Please deposit funds to your Ethereum address: ${deployerAccount.address}`);
        return;
    }
    ...
    console.log(await contract.methods.YOUR_FUNCTION_NAME().call());


    // const tx = new web3.eth.Contract(CompiledContractArtifact.abi).deploy({
