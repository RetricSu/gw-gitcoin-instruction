...
```sh
ckb-cli --url <TESTNET_RPC_URL> wallet get-capacity --address <YOUR_TESTNET_ADDRESS>
mh
```

*Note: as `--url` value (`<TESTNET_RPC_URL>`) you can pass ready-to-use remote testnet node URL: `http://3.235.223.161:18114` or run your local testnet node and pass its address.*

**Attention!**

**If might take a very long time (hours) to sync first time with indexer when running this command. We highly recommend you use checkpoint data for ckb-cli before using `get-balance` command. [Click here to check how to make syncing ckb-cli faster.](../component-tutorials/5.setup.testnet.node.md#ckb-cli)**
